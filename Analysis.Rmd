...
par(cex.main=0.25)
heatmap.2(-Xne[order(-rowSums(Xne))[2:22],], col=redgreen(70), distfun=rank.cor.dist, 
...
Rad_KO_Interaction<-function(cellType, trimOutliers=FALSE){
  ...
  cbind(rownames(mtx), X[fNames %in% rownames(mtx), P$Cell==cellType])
}
```
...
Rad_KO_Interaction("MCF7", trimOutliers=F)
```
...
Rad_KO_Interaction("MCF7", trimOutliers=T)
```
